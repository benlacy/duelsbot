...
def register_leave_command(bot: commands.Bot):
    ...
    async def leave(ctx):
        print(f"[LEAVE] Command invoked by user {ctx.author} ({ctx.author.id}) in #{ctx.channel.name}")

        if ctx.channel.name != "queue-here":
            print("[LEAVE] Command used in wrong channel.")
            return
        ...
        cursor = conn.cursor()
        print(f"[LEAVE] Checking queue status for user_id={user_id}")
        ...
        if not row:
            print("[LEAVE] User not found in database.")
        ...
        queue_status = row[0]
        print(f"[LEAVE] Current queue_status for {user_id}: {queue_status}")

        if queue_status != "IN_QUEUE":
            print("[LEAVE] User is not in queue.")
            await ctx.message.delete()
        ...
        print(f"[LEAVE] Updating user {user_id} to IDLE.")
        ...
        print(f"[LEAVE] Sending queue update embed to channel #{ctx.channel.name}")

        await ctx.message.delete()
        await ctx.author.send("✅ You have left the queue.")
        await ctx.channel.send(embed=embed)
        print(f"[LEAVE] User {user_id} has successfully left the queue.")
