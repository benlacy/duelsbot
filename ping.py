...
def register_ping_command(bot: commands.Bot):
    ...
    async def ping(ctx):
        ...
        logging.info(f"User {ctx.author} ({user_id}) used !ping from {location}")
        ...
        # Update ping column
        cursor.execute("UPDATE players SET ping = 1 WHERE discord_id = ?", (user_id,))
        conn.commit()
        conn.close()
        logging.info(f"Set ping = 1 for {user_id}")

        # Add ping role
        ping_role = discord.utils.get(guild.roles, name="PING")
        if ping_role:
            await member.add_roles(ping_role)
            await ctx.send("✅ You will now be pinged for future matches.")
            logging.info(f"Gave PING role to {user_id}")
        else:
            await ctx.send("⚠️ 'PING' role not found in this server.")
            logging.warning(f"'PING' role not found while processing !ping for {user_id}")
